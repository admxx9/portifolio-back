...
export default async function handler(request, response) {
  ...
  if (action === "login" && request.method === "POST") { const data = await body(request); const result = await pool.query("SELECT * FROM portfolio_users WHERE email=$1", [data.email?.trim().toLowerCase()]); if (!result.rows[0] || !(await matches(data.password || "", result.rows[0].password_hash))) return json(response, { error: "E-mail ou senha inválidos." }, 401); return json(response, { user: { id: result.rows[0].id, email: result.rows[0].email } }, 200, { "Set-Cookie": cookie("portfolio_session", session(result.rows[0]), 2592000) }); }
